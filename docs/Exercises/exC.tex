...
\include{operational-semantics.tex}
...
\begin{prooftree}
    \AxiomC{}
    \RightLabel{Act}
    \UnaryInfC{$a_m.\enc P\{^m/_x\}\cne \xrightarrow{a_m} \enc P\{^m/_x\}\cne$}
    \RightLabel{Sum}
...
\end{prooftree}
(Here $\alpha = a(m)$, so $\hat\alpha = a_m$.)
...
\end{prooftree}
(Here $\alpha = \bar{a}(m)$, so $\hat\alpha = \bar{a}_m$.)
...
\textbf{Ren} $P[f] \xrightarrow{f(\alpha)} P'[f]$:\\[1em]
\begin{prooftree}
    \AxiomC{$P \xrightarrow{\alpha} P'$}
    \RightLabel{IH}
    \UnaryInfC{$\enc P\cne \xrightarrow{\hat\alpha} \enc P'\cne$}
...
\end{prooftree}
Where $f'(a_m)=f(a)_m$ and $\widehat{f(\alpha)}=f'(\hat\alpha)$.
The equality $\widehat{f(\alpha)} = f'(\hat\alpha)$ ensures the commutation of encoding and renaming.
\textit{Proof by cases on $\alpha$:}
\[
\begin{array}{lll}
\alpha = a(m):        & f'(\hat\alpha) = f'(a_m) = f(a)_m              & = \widehat{f(a)(m)}            = \widehat{f(\alpha)} \\
\alpha = \bar{a}(m): & f'(\hat\alpha) = f'(\bar{a}_m) = \overline{f(a)}_m & = \widehat{\overline{f(a)}(m)} = \widehat{f(\alpha)} \\
\alpha = \tau:         & f'(\tau) = \tau                                  & = \widehat{f(\tau)}            = \widehat{f(\alpha)}
\end{array}
\]

\textbf{Const} $K(e_1,\ldots,e_h) \xrightarrow{\alpha} P'$, $e_i\downarrow n_i$:\\[1em]
We are doing rule induction on the given derivation of $K(e_1,\ldots,e_h) \xrightarrow{\alpha} P'$.
The last rule fired is CCS-VP Const, whose shape is:
\begin{prooftree}
    \AxiomC{$P\{^{n_1}/_{x_1},\ldots\} \xrightarrow{\alpha} P'$}
    \RightLabel{Const}
    \UnaryInfC{$K(e_1,\ldots,e_h) \xrightarrow{\alpha} P'$}
\end{prooftree}
This tells us that $P'$ is determined by the premise, and that $P\{^{n_1}/_{x_1},\ldots\} \xrightarrow{\alpha} P'$ holds with a strictly shorter derivation. The IH applies to that premise:
\begin{prooftree}
    \AxiomC{$P\{^{n_1}/_{x_1},\ldots\} \xrightarrow{\alpha} P'$}
    \RightLabel{IH}
    \UnaryInfC{$\enc P\{^{n_1}/_{x_1},\ldots\}\cne \xrightarrow{\hat\alpha} \enc P'\cne$}
    \RightLabel{Const}
    \UnaryInfC{$K_{n_1,\ldots} \xrightarrow{\hat\alpha} \enc P'\cne$}
    \RightLabel{$=$}
    \UnaryInfC{$\enc K(e_1,\ldots,e_h)\cne \xrightarrow{\hat\alpha} \enc P'\cne$}
\end{prooftree}
...
\end{prooftree}
$\enc a(x).P\cne = \textstyle\sum_{n\in\N} a_n.\enc P\{^n/_x\}\cne$; the unique transition selects branch $m$ via Act and Sum:
\begin{prooftree}
    \AxiomC{}
    \RightLabel{Act}
    \UnaryInfC{$a_m.\enc P\{^m/_x\}\cne \xrightarrow{a_m} \enc P\{^m/_x\}\cne$}
    \RightLabel{Sum (branch $m$)}
    \UnaryInfC{$\textstyle\sum_{n\in\N} a_n.\enc P\{^n/_x\}\cne \xrightarrow{a_m} \enc P\{^m/_x\}\cne$}
\end{prooftree}
So $Q = \enc P\{^m/_x\}\cne$.\\
...
The equality $\widehat{f(\alpha)} = f'(\hat\alpha)$ ensures the commutation of encoding and renaming: the CCS output label $f'(\hat\alpha)$ matches $\widehat{f(\alpha)}$ as required.
\vspace{.5cm}
...
The transition follows from unfolding the constant $K_{n_1,\ldots}$ and applying the IH to its body; derivation depth strictly decreases at each unfolding step.\\
